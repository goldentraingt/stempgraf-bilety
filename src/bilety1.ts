...
const textOverlay = async (path: string, outputFile: string, x: number, y: number, rotate: boolean, ticketNumber: number) => {
	...
	await image.print(font, x, y, '0' + ticketNumber);
...
const generatePage = async (startNumber: number, pageNumber: number) => {
	...
	await fs.copy(`./template/images`, `./output/${startNumber}_${pageNumber}/images`, { recursive: true }).catch((_) => _);

	for (let i = 0; i < data.length; i++) {
		const asidePath = './template/aside.jpg';
		const asideOutputFile = `./output/${startNumber}_${pageNumber}/images/${data[i][0]}.jpg`;
		const asideX = 1;
		const asideY = 3;
		...
		const mainOutputFile = `./output/${startNumber}_${pageNumber}/images/${data[i][1]}.jpg`;
		const mainX = 120;
		const mainY = 50;
	...
	await fs.copy(`./template/bilety1_2021.htm`, `./output/${startNumber}_${pageNumber}/bilety1_2021.htm`).catch((_) => _);
};
...
(async () => {
	...
	const startNumber = 57600;
	const endNumber = 79600;

	await fs.mkdir('./output/_bilety1_pdf/blocks').catch((_) => _);

	for (let i = startNumber; i <= endNumber; i += 1200) {
		// for (let j = 1; j <= 100; j++) {
		// 	const isExist: Boolean = await fs
		// 		.access(`./output/${i}_${j}/bilety1_2021.htm`)
		// 		.then(() => true)
		// 		.catch(() => false);
		...
		// 		.access(`./output/_bilety1_pdf/${i}_${j}_bilety1_2021.pdf`)
		// 		.then(() => true)
		// 		.catch(() => false);

		// 	if (!isAccesstoPdf) {
		// 		await page.goto(`file:////${join(__dirname, '../', 'output', `${i}_${j}`, 'bilety1_2021.htm')}`);

		// 		await page.pdf({
		// 			format: 'a4',
		// 			path: `./output/_bilety1_pdf/${i}_${j}_bilety1_2021.pdf`,
		// 		});
		...
		const pdfFiles: string[] = await fs.readdir('./output/_bilety1_pdf');
		...
		if (pdfFilesFromStartNumber.length === 100) {
			...
			firstPart.map(async (el) => {
				merger.add(`./output/_bilety1_pdf/${el}`);
			});

			await merger.save(`./output/_bilety1_pdf/blocks/${i}_1-50.pdf`);

			secondPart.map(async (el) => {
				merger.add(`./output/_bilety1_pdf/${el}`);
			});

			await merger.save(`./output/_bilety1_pdf/blocks/${i}_51-100.pdf`);
